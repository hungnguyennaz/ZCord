...
public abstract class Configuration implements ProxyConfig
{
    ...
    public void load()
    {
        ...
        disabledCommands.remove( "bungee" ); //ZCord
        disabledCommands.remove( "ZCord" ); //ZCord
        ...
        for ( ListenerInfo listener : listeners )
        {
            ...
            for ( String server : listener.getForcedHosts().values() )
            {
                ...
                {
                    ProxyServer.getInstance().getLogger().log( Level.INFO, "Forced host server {0} is not defined", server );
                }
            }
        }
    }
