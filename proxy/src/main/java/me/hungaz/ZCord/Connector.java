...
import me.hungaz.ZCord.caching.CachedCaptcha.CaptchaHolder;
...
public class Connector extends MoveHandler
{
    ...
    private int aticks = 0, sentPings = 0, attemps = 2;
    @Getter
    ...
    public Connector(UserConnection userConnection, ZCord ZCord)
    {
        this.ZCord = ZCord;
        this.state = this.ZCord.getCurrentCheckState();
    ...
    public void spawn()
    {
        this.ZCord.incrementBotCounter();
        ...
        switch ( state )
        {
            ...
            case CAPTCHA_POSITION:
                if(Settings.IMP.LOG_ANTIBOT_CHECKS) {
                    String info = "(ZCord) [" + name + "|" + ip + "] left from server during check";
    ...
    public void completeCheck()
    {
        ...
        if ( ZCord.checkBigPing( totalping / ( devide <= 0 ? 1 : devide ) ) )
        {
        ...
        ZCord.saveUser( getName(), IPUtils.getAddress( userConnection ), true );
        ...
        markDisconnected = true;
        if(Settings.IMP.LOG_ANTIBOT_CHECKS) {
            LOGGER.log(Level.INFO, "[ZCord] Player (" + name + "|" + ip + ") successfully passed the test!");
        }
    }
    ...
    private void handleChat(String message)
    {
        if ( state != CheckState.CAPTCHA_ON_POSITION_FAILED )
        {
            ...
            {
                sendMessage( ( attemps == 1 ? PacketsPosition.CAPTCHA_FAILED_2_MSG : PacketsPosition.CAPTCHA_FAILED_1_MSG ) );
                sendCaptcha();
            } else
            {
    ...
    public void failed(KickType type, String kickMessage)
    {
        ...
        markDisconnected = true;
        if(Settings.IMP.LOG_ANTIBOT_CHECKS) {
            LOGGER.log(Level.INFO, "(ZCord) [" + name + "|" + ip + "] check failed: " + kickMessage);
        }
    }
